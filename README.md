...
## Konfigurering
Det finns en lista över bilder i `js/carousel.js` som behöver uppdateras om bilder läggs till i `images/`. Motsvarande ID behöver också läggas till i `css/carousel.js` (t.ex. `.carousel__slide img.id1` för bild `images/1.*`) med en procent-angivelse som motsvarar bildens vertikala centrum, så att skalningen blir korrekt i breda fönster.
